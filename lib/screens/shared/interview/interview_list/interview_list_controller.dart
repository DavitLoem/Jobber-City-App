...
class InterviewListViewController extends GetxController {
  ...
  List<InterviewModel> get visibleInterviews {
    if (filter.value == _InterviewFilter.upcoming) {
      ...
      list.sort((a, b) => a.scheduledAt.compareTo(b.scheduledAt)); // soonest first
      return list;
    }
    final list = interviews.where((i) => !i.isUpcoming).toList();
    list.sort((a, b) => b.scheduledAt.compareTo(a.scheduledAt)); // most recent first
    return list;
  }
  ...
  Future<void> fetchInterviews() async {
    ...
    errorMessage.value = '';
    try {
      final result = await _interviewService.listInterviews(status: 'all', limit: 50);
      interviews.assignAll(result);
    } catch (e) {
      errorMessage.value = 'Could not load your interviews. Pull down to try again.';
