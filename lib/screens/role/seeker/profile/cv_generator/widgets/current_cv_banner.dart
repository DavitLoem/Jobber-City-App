...
class CurrentCvBanner extends StatelessWidget {
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  'You already have a CV'.tr, // 🟢 Added .tr
  ...
  String _subtitle() {
    final templateName = cv.templateId != null
        ? '${cv.templateId![0].toUpperCase()}${cv.templateId!.substring(1)}'
        : 'CV';

    if (cv.generatedAt == null) {
      return '@template template'.trParams({
    ...
    return '@template template · Generated @date'.trParams({
      // 🟢 Added .trParams
      'template': templateName,
      'date': date,
    });
