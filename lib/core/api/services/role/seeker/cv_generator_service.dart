...
class CvGeneratorService {
  ...
  Future<GeneratedCvModel> generateCv(String templateId) async {
    ...
    debugPrint("Generate CV Response:  $response");
  ...
  Future<String> previewCvHtml(String templateId) async {
    // 🎯 response ទីនេះគឺជាទិន្នន័យ (HTML String) ដែលបានពី ApiClient
    final response = await _apiClient.get(
      '$_endpoint/preview/$templateId',
      options: Options(responseType: ResponseType.plain),
    );

    // គ្រាន់តែបំប្លែងទៅជា String ជាការស្រេច មិនបាច់ .data ទៀតទេ
